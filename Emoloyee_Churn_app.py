from re import M
import streamlit as st
import pandas as pd
import numpy as np
from PIL import Image


st.markdown("<h1 style='text-align: center; color: Indigo;'>This is an Employee Churn Prediction Application</h1>", unsafe_allow_html=True)



img = Image.open("churn.jpg")
st.image(img, width=325)

st.success('##### The model was trained with the following parameters of nearly 15000 employees.')


st.markdown("##### 1. Satisfaction level: Employee satisfaction point, which ranges from 0-100 :smiley:")
st.markdown("##### 2. Performance: Evaluated performance, which also ranges from 0-100 :male-detective:")
st.markdown("##### 3. Working years: The number of years spent by an employee in the company :older_adult:")
st.markdown("##### 4. Working hours: How many hours an employee worked in a month? :stopwatch:")
st.markdown("##### 5. Number of projects: How many projects the employee is assigned to? :open_file_folder:")
st.markdown("##### 6. Work accident: Whether an employee has had a work accident or not :face_with_head_bandage:")
st.markdown("##### 7. Promotion: Has the employee  had a promotion in the last 5 years :gift:")
st.markdown("##### 8. Departments: Employee's working department/division :female-mechanic:")
st.markdown("##### 9. Salary: Salary level of the employee; low, medium and high :moneybag:")
st.markdown("##### 10. Left: Whether the employee has left the company or not :slightly_frowning_face:")

st.markdown("###")
st.success("##### Please enter the information of the employee for prediction from the left side bar and below")

import pickle
filename = 'Random_Forest.pkl'
model = pickle.load(open(filename, 'rb'))

st.markdown("##### Please select a department")
Department = st.selectbox("Departments",
        ('sales', 'accounting', 'hr', 'technical', 'support', 'management',
       'IT', 'product_mng', 'marketing', 'RandD')
    )

st.markdown("###")
st.markdown("##### Please select a salary")
Salary = st.radio(
    "Salary",
    ('low', 'medium', 'high')
    )


st.markdown("###")
st.markdown("##### Has the employee ever had a work accident?")
Work_accident = st.radio(
    "Accident",
    ('No', 'Yes')
    )   
if Work_accident == "Yes":   
    Work_accident = 1 
elif Work_accident == "No":     
    Work_accident = 0

st.markdown("###")
st.markdown("##### Has the employee received any promotions in the past five years?")   
promotion_last_5years = st.radio(
    "Promotion",
    ('Yes', 'No')
    )   
if promotion_last_5years == "Yes":   
    promotion_last_5years = 1 
elif promotion_last_5years == "No":     
    promotion_last_5years = 0


satisfaction_level = st.sidebar.slider("Satisfaction level:",min_value=0, max_value=100)

last_evaluation = st.sidebar.slider("Performance:",min_value=0, max_value=100)

time_spend_company = st.sidebar.slider("Working years:",min_value=1, max_value=10)

average_montly_hours = st.sidebar.slider("Working hours:",min_value=40, max_value=310)

number_project = st.sidebar.slider("Number of projects:",min_value=1, max_value=10)



   
my_dict = {
    'departments': Department,
    'salary': Salary,
    'satisfaction_level': (satisfaction_level/100) ,
    'last_evaluation': last_evaluation/100,
    'number_project': number_project,
    'average_montly_hours': average_montly_hours,
    'time_spend_company': time_spend_company,
    'Work_accident': Work_accident,
    'promotion_last_5years': promotion_last_5years  
}

st.info("The Predictors for Your Employee")
my_dict

df=pd.DataFrame.from_dict([my_dict])



if st.button("Predict"): 
    pred = model.predict(df)
    if int(pred[0]) == 1:
        st.error("Your employee is very likely to leave the company")
    else:
        st.success("Your employee is loyal to the company")




